...
def plotImag(array, outPNG, band, date, tickets, cdict):
    ...
    #dpi10 = 10*dpi
    #fig = plt.figure(figsize=figsize, dpi=dpi10)
    ...
    plt.yticks([])
    #plt.title(date)
    res=360.0/float(len(array[0]))  # for 0.05
    res=0.1*360.0/float(len(array[0]))  # for 0.5 (dirty)
...
# band_5 = 1240nm
# band_6 = 1640nm
wvls = [' (645nm)', ' (865nm)', ' (470nm)', ' (555nm)', ' (1240nm)', ' (1640nm)']
...
for band in bands:
    ...
    plotImag(data,outdir+'/'+name+"_"+bandDisplay[i]+".png", bandDisplay[i]+wvls[i], dateStr, tickets, cdict)
    i+=1
