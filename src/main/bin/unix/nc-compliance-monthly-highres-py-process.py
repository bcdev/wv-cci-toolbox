...
def set_surface_type_flag(dst, src):
    ...
    # 4. make partly_cloudy_over_land (5) if surface_type_MAX = 2  but surface_type_MIN < 2 (cloudy on at least
    #    one day but not all days, refers to b))
...
def set_global_attributes(sensor, datestring, dst, month, year,
...
def run(args):
    ...
    set_global_attributes(sensor, datestring, dst, month, year, lat_min, lat_max, lon_min, lon_max, version, nc_infile, nc_outfile)
...
if __name__ == "__main__":
    ...
    print("STARTING nc-compliance-monthly-highres-py-process.py", file=sys.stderr)
    print('Working dir: ', os.getcwd())

    if len(sys.argv) != 12:
        print('Usage:  python nc-compliance-monthly-highres-py-process.py <nc_infile> <landmask_file> <roi> <sensor> '
              '<year> <month> <lat_min> <lat_max> <lon_min> <lon_max> < product version> ')
        sys.exit(-1)
