...
if len(sys.argv) != 8:
    print ('Usage:  python nc-compliance-update-dataset1-process.py <nc_infile> <sensor> <year> <month> <day> <resolution> < product version>')
    sys.exit(-1)
...
with Dataset(nc_infile) as src, Dataset(outpath, 'w', format='NETCDF4') as dst:
    ...
    dst.setncattr('sensor', 'MERIS, MODIS, CM SAF HOAPS')
    ...
    for name, variable in src.variables.iteritems():
        if name.find('sigma') == -1 and (name.find("tcwv") != -1 or name == 'crs'):
