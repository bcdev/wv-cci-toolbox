...
def get_ds_olci_tcwv_counts(nc_l3_file):
    """
    Gets OLCI L3 file with tcwv counts
    :return: ds_hoaps: hoaps nc4 dataset
    """
    ...
    if nc_l3_file == 'none':
        ...
        if nc_l3_file:
            try:
                ds_counts = Dataset(nc_l3_file)
            except OSError:
                print('Cannot read original OLCI L3 TCWV counts file')
                ds_counts = None
        return ds_counts
...
def run(args):
    ...
    nc_infile = args[1]
    landmask_file = args[4]
    landcover_file = args[5]
    sensor = args[6]
    year = args[7]
    month = args[8]
    day = args[9]
    ...
    # Reduced OLCI L3 inputs with TCWV counts for num_obs correction...
    ds_olci_a_tcwv_counts = get_ds_olci_tcwv_counts(args[2])
    ds_olci_b_tcwv_counts = get_ds_olci_tcwv_counts(args[3])
    ...
    print("FINISHED nc-compliance-daily-corr-numobs-olci.py...", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 13 and len(sys.argv) != 14:
        print(
            'Usage:  python nc_compliance_daily.py <nc_infile> <olci_a_counts_infile> <olci_b_counts_infile> ' +
            '<landmask_file> <landcover_file> <sensor> <year> <month> <day> ' +
            '<resolution> <product version> <seaice_mask_file> <hoaps_l3_file>')
        print('NOTE: <olci_a_counts_infile>, <olci_b_counts_infile>, <seaice_mask_file> <hoaps_l3_file> can be "none" ')
        sys.exit(-1)
