...
def set_surface_type_flag(dst, src, ds_landmask):
    """
    ...
    hoaps_surface_type_flag_arr = rescale_hoaps(hoaps_surface_type_flag_arr_src)
...
def init_nc_compliant_product(datestring, roi, sensor, version):
    ...
    nc_outfile = 'ESACCI-WATERVAPOUR-L3' + l3_suffix + '-TCWV-' + sensor + '-' + '001deg-' + roi + '-' + datestring + '-fv' + \
                 version + '.nc'
...
def set_global_attributes(sensor, datestring, dst, day, month, year,
                          lat_min, lat_max, lon_min, lon_max,
                          version, nc_infile, nc_outfile):
    """
    ...
    dst.setncattr('geospatial_lat_min', lat_min)
    dst.setncattr('geospatial_lat_max', lat_max)
    dst.setncattr('geospatial_lon_min', lon_min)
    dst.setncattr('geospatial_lon_max', lon_max)
    ...
    spatial_resolution = '1.12km at Equator'
...
def run(args):
    ...
    month = args[6]
    day = args[7]
    lat_min = float(args[8])
    lat_max = float(args[9])
    lon_min = float(args[10])
    lon_max = float(args[11])
    version = args[12]
    ...
    set_global_attributes(sensor, datestring, dst, day, month, year, lat_min, lat_max, lon_min, lon_max, version, nc_infile, nc_outfile)
    ...
    set_surface_type_flag(dst, src, ds_landmask)
...
if __name__ == "__main__":
    ...
    print("STARTING nc-compliance-highres-py-process.py", file=sys.stderr)
    print('Working dir: ', os.getcwd())

    if len(sys.argv) != 13:
        print(
            'Usage:  python nc-compliance-highres-py-process.py <nc_infile> <landmask_file> <roi> <sensor> '
