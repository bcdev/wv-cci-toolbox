...
if sensor == 'meris':
    ...
    source = ' OLCI RR L1b'
    start_index = nc_infile.find("S3A_OL_1_")
    ...
    if variable.find("scan_time") == -1:
        if variable.find("metadata") != -1 or variable.find("_mask") != -1:
            ...
            if variable.find("surface_type_flags") != -1:
                ...
                # over land, ocean, sea ice, but not cloudy (< 8). These remaining pixels must be l1_invalid, set to -128
                    
                surface_type_flags_arr = src.variables["surface_type_flags"][:,:]
                quality_flags_arr = src.variables["tcwv_quality_flags"][:,:]
                tmparr = np.copy(surface_type_flags_arr)
                tcwv_arr = src.variables["tcwv"][:,:]
                tmparr[np.where((src.variables["surface_type_flags"][:,:] < 8) &
                                (src.variables["tcwv_quality_flags"][:,:] > 2))] = -128
                dst.variables[variable][:,:] = tmparr[:,:]                  
