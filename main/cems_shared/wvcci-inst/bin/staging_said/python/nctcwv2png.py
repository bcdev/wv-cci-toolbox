...
def plotImag(array, outPNG, band, date, tickets, cdict):
    ...
    cbaxes = fig.add_axes([0.45, 0.9, 0.45, 0.05]) # left, bottom, width, height
    ...
    ax1.text(0.3, 0.9, 'TCWV (mm) - ' + sensor + ' - ' + year + month,
        verticalalignment='bottom', horizontalalignment='right',
        transform=ax1.transAxes,
...
inFile = sys.argv[4]
outDir = sys.argv[5]
min_max = sys.argv[6].split(',');
lutColor = sys.argv[7];
size = sys.argv[8];
colorTxt = sys.argv[9]
print('colorTxt: ', colorTxt)
bands = sys.argv[10].split(',')
print('bands: ', bands)
bandsDis = sys.argv[10]
...
for band in bands:
    ...
    data=np.where(data>min[i], data, -10.0)
