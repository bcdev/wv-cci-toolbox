    ...
    for in2 in range(n2):
        ...
        for inn in range(nn):
            ...
            for iex in range(ex):
                ...
                # max_index_lut = min(adre[in2], lut.shape[0]-1)
                # jaco[iex, inn] += lut[max_index_lut, iex] * weght  * 1.E18   # allows for testing with small LUT subset
        for iex in range(ex):
            out[iex] += lut[adre[in2], iex] * oweg[in2]
            # max_index_lut = min(adre[in2], lut.shape[0]-1)
            # out[iex] += lut[max_index_lut, iex] * oweg[in2]
    return jaco, out, coor
...
def generate_itp_pn_jac_parallel(lut, sca=None, batch=False):
    ...
    #la, li, lu =_function_itp_np_j_b(wo, 0, put[0:5], sha, mul, exx, sca)
    #bla = 1
    ### end test
