...
import numpy as np
import optimal_estimation_core_pure_python_cawa as oecpy
...
def test():
    ...
    for func_key in ['a', 'b', 'c']:
        ...
        print
        'XA', XA[func_key], '--> YA:', FUNC[func_key](XA[func_key])
        print
        'SE'
        print
        SE[func_key]
        ...
        for i, meth in enumerate(method):
            ...
            if i > 0:
                print
                '    diag avKern: ', [erg.a[j, j] for j in range(erg.a.shape[0])]
                print
                '    diag ret.er: ', [erg.sr[j, j] for j in range(erg.sr.shape[0])]
            print
            '         nitter: ', erg.ni
            print
            '           cost: ', erg.cost
            print
            '    f(retrie X): ', FUNC[func_key](erg.x)
            print
            '    -'
        ...
        print
        print
        'Test with x =', XT[func_key], '-->  disturbed y=', yt
        for i, meth in enumerate(method):
            if i != 2 and func_key == 'c':
                continue
            erg = inv_func(yt, full=True, sa=SA[func_key], se=SE[func_key], xa=XA[func_key], eps=0.001, method=i)
...
def test_lm():
    ...
    inv_func = my_inverter(FUNC[func_key], AA[func_key], BB[func_key], dtype=dtype)
    print
    inv_func(yt, full=True, sa=SA[func_key], se=SE[func_key], xa=XA[func_key], eps=0.001, method=2, maxiter=10)
    print
    inv_func(yt, full=True, sa=SA[func_key], se=SE[func_key], xa=XA[func_key], eps=0.001, method=5, maxiter=100)
